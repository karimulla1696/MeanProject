...
//install and import jsonwebtoken for generate token
...
let checkToken=(req,res,next)=>{
    ...
    if(token!==undefined){
        ...
        {
            //if anything goes wrong ,return an error immediately before passing control control to next
            if(err){
                return res.json({
      ...
      if(err){
          ...
          bcrypt.compare(req.body.password,data[0].password,(err,success)=>{
              if(err){
                ...
                var jwtBearerToken=jwt.sign({name:req.body.name},k,{expiresIn:60*60*24*7});
...
app.get('/admin/empolyeedetails',checkToken,(req,res,next)=>{
...
app.get('/user/leave',checkToken,(req,res,next)=>{
    dbo.collection('employeeleave').find({name:s}).toArray((err,data)=>{
        if(err)
        {
            console.log(err);
        }
...
app.get('/admin/empolyeeleave',checkToken,(req,res,next)=>{
...
app.get('/admin/empolyeepayslip',checkToken,(req,res,next)=>{
    dbo.collection('employeedetails').find({name:{$ne:"admin"}}).toArray((err,data)=>{
        if(err){
            console.log(err);
        }
...
app.get('/user/payslip',checkToken,(req,res,next)=>{
   ...
   },(err,sucess)=>
   {
      ...
      {     
          // res.json("sucessfully add the employee atendance to empolyee colecction ")
           dbo.collection("attendancedetails").find({name:s}).toArray((err,data)=>
           {
               if(err)
                 {
...
//GETTING ATTENDANCE DEATILS FRPM ATTENDANCE COLLECTION To Admin
     app.get("/admin/empolyeeattendance",checkToken,(req,res)=>
                  {    
...
app.get("/user/attendance",checkToken,(req,res)=>
{
    dbo.collection("attendancedetails").find({name:s}).toArray((err,data)=>
        {
            if(err)
